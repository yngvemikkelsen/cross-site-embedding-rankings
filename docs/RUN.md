...
# pin versions in env/requirements.txt first (see that file's header), then:
...
# Emits attrition counts — paste them into manifests/sampling_manifest.md.
```
...
## 4. Primary statistics (Tables 2-4)

```bash
python src/two_site_v2_stats.py
# Cross-site Kendall tau (Table 2), selection regret (Table 3), variance decomposition
# full-panel + contrastive with patient-clustered bootstrap CIs (Table 4).
```

## 5. Robustness (Table 5 + diagnostics)
